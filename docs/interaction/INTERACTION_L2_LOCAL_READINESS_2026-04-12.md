...
- 生成时间(UTC): 2026-04-12T09:47:55.436363+00:00
- 扫描模式: full
- 总体结论: **PASS**
...
| RPI | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 部分 | 已完成 | 部分 |
...
| # | 互作 | 缺口 | 分级 | 工作量 | 建议 |
|---|---|---|---|---|---|
| 1 | PPI | 更新频率为部分 | 可延期 | S | 按统一 L2 字段规范补齐并在本地复跑 QA。 |
| 2 | PSI | 更新频率为部分 | 可延期 | S | 按统一 L2 字段规范补齐并在本地复跑 QA。 |
| 3 | RPI | 更新频率为部分 | 可延期 | S | 按统一 L2 字段规范补齐并在本地复跑 QA。 |
...
| rpi_edge_evidence_hierarchy_ge_0_99 | RPI evidence.edge_id 与 edges.edge_id 连接率 >= 0.99 | PASS |
| interaction_schema_uniformity | 三类互作均满足统一 L2 字段口径 | PASS |
| zero_blocking_gaps | Top gaps 中阻塞级缺口为 0 | PASS |

## 5) 收口建议

1. 已完成三类互作本地收口，建议冻结当前 manifest 与 gates 作为上传前基线。
2. 维持统一 L2 字段口径（method/reference/score/source_version/fetch_date），后续增量按同合同校验。
3. 当前仅剩可延期项（更新频率分层），可在下一轮引入多时间窗快照后提升为“已完成”。
